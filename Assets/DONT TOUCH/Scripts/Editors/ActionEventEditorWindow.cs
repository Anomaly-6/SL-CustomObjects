...
public class ActionEventEditorWindow : EditorWindow
{
    ...
    private string GetEventDisplayName(int index)
    {
        ...
        SerializedProperty displayNameProperty = eventElement.FindPropertyRelative(nameof(ActionEventList.DisplayName));
    ...
    private void OnGUI()
    {
        if (_target == null)
        {
            EditorGUILayout.HelpBox("Select an object with ActionEvents and open this window from its inspector.", UnityEditor.MessageType.Info);
        ...
        {
            EditorGUILayout.HelpBox("No action event lists found. Define ActionEvents in component code.", UnityEditor.MessageType.Warning);
        }
        else
        {
    ...
    private static void DrawActionElement(SerializedProperty actionsProperty, Rect rect, int index, bool isActive, bool isFocused)
    {
        if (actionsProperty == null || index < 0 || index >= actionsProperty.arraySize)
            return;
        ...
        {
            expandedProperty.boolValue = EditorGUI.Foldout(new Rect(rect.x, y, 20, EditorGUIUtility.singleLineHeight), expandedProperty.boolValue, GUIContent.none);
        }
        else
        {
        ...
        if (nameProperty != null)
        {
            ...
            EditorGUI.PropertyField(new Rect(rect.x, y, rect.width, EditorGUIUtility.singleLineHeight), nameProperty);
            y += LineWithSpacing();
        }
        ...
        EditorGUI.PropertyField(new Rect(rect.x, y, rect.width, EditorGUIUtility.singleLineHeight), actionDelayProperty);
        y += LineWithSpacing();
        
        if (actionType == ActionType.Command || actionType == ActionType.Audio)
        {
            if (paramTypeProperty.intValue != 0) paramTypeProperty.intValue = 0;
            EditorGUI.PropertyField(new Rect(rect.x, y, rect.width, EditorGUIUtility.singleLineHeight), valueProperty);
            return;
        }
        ...
        if (animator == null)
        {
            ...
            EditorGUI.HelpBox(new Rect(rect.x, y, rect.width, HelpBoxHeight), "Target must have an Animator component.", UnityEditor.MessageType.Info);
            return;
        }
    ...
    private static void DrawComponentPropertySelection(SerializedProperty paramProperty, SerializedProperty valueProperty, GameObject target, float x, ref float y, float width)
    {
        if (target == null)
        {
            EditorGUI.PropertyField(new Rect(x, y, width, EditorGUIUtility.singleLineHeight), paramProperty, new GUIContent("Property Name"));
            y += LineWithSpacing();
            EditorGUI.PropertyField(new Rect(x, y, width, EditorGUIUtility.singleLineHeight), valueProperty, new GUIContent("New Value"));
            y += LineWithSpacing();
            return;
        }
        ...
        int newIndex = EditorGUI.Popup(new Rect(x, y, width, EditorGUIUtility.singleLineHeight), "Property", selectedIndex, displayOptions.ToArray());
    ...
    private static void DrawPropertyValueFieldByType(Type type, SerializedProperty valueProperty, ref float y, float x, float width)
    {
        if (type == typeof(bool))
        {
            bool boolValue = string.Equals(valueProperty.stringValue, bool.TrueString, StringComparison.OrdinalIgnoreCase);
            int selected = EditorGUI.Popup(new Rect(x, y, width, EditorGUIUtility.singleLineHeight), "Value", boolValue ? 1 : 0, new[] { bool.FalseString, bool.TrueString });
        ...
        {
            int.TryParse(valueProperty.stringValue, NumberStyles.Integer, CultureInfo.InvariantCulture, out int intValue);
            int newInt = EditorGUI.IntField(new Rect(x, y, width, EditorGUIUtility.singleLineHeight), "Value", intValue);
        ...
        {
            float.TryParse(valueProperty.stringValue, NumberStyles.Float, CultureInfo.InvariantCulture, out float floatValue);
            float newFloat = EditorGUI.FloatField(new Rect(x, y, width, EditorGUIUtility.singleLineHeight), "Value", floatValue);
        ...
        else if (type == typeof(Color))
        {
            ...
            if (!string.IsNullOrEmpty(valueProperty.stringValue))
            {
                ...
                {
                    ColorUtility.TryParseHtmlString(valueProperty.stringValue.StartsWith("#") ? valueProperty.stringValue : "#" + valueProperty.stringValue, out colorValue);
                }
            }
            colorValue = EditorGUI.ColorField(new Rect(x, y, width, EditorGUIUtility.singleLineHeight), new GUIContent("Value"), colorValue, true, true, true);
                string colorStr;
    ...
    private static void DrawParameterPopup(SerializedProperty paramProperty, AnimatorParameter[] parameters, float x, float y, float width)
    {
        ...
        int newIndex = EditorGUI.Popup(new Rect(x, y, width, EditorGUIUtility.singleLineHeight), "Param", selectedIndex, options);
    ...
    private static void DrawAnimationValueField(AnimatorParameterType parameterType, SerializedProperty valueProperty, ref float y, float x, float width)
    {
    ...
    private static bool TryGetAnimatorParamType(Animator animator, string parameterName, out AnimatorParameterType parameterType)
    {
        parameterType = AnimatorParameterType.Trigger;
